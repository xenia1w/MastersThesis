"""
Earnings21 full-call prompted evaluation.

For each call:
  1. Loads the pre-built LLM profile from --profiles-dir.
  2. Splits at the profile boundary: segments[n_profile:] form the evaluation window.
  3. Loads audio from segments[n_profile-1].end_ts to end of file.
  4. Runs Whisper with prompt_ids injected via generate_kwargs.
  5. Computes WER and entity EER against the concatenated NLP reference text.

Usage:
    uv run -m src.lexical_stylistic_prompting.pipeline.earnings21_fullcall_prompted_eval \
        --data-dir     data/raw/earnings21 \
        --profiles-dir data/processed/lexical_stylistic_prompting/profiles \
        --output       data/processed/lexical_stylistic_prompting/earnings21_fullcall_metadata_only/prompted_all.csv \
...
class FullCallPromptedRow(BaseModel):
    call_id: str
    strategy: str
    prompt: str
    n_eval_segments: int
...
def _load_eval_audio(
    call: Earnings21Call, split_ts: float, end_ts: float | None = None
) -> np.ndarray:
...
def evaluate_call(
    ...
    result = asr_pipe(audio, generate_kwargs={"prompt_ids": prompt_ids})
...
def _save_csv(rows: list[FullCallPromptedRow], path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(FullCallPromptedRow.model_fields))
        writer.writeheader()
        for row in rows:
...
def main() -> None:
    ...
    parser.add_argument("--strategy",     required=True, choices=[s.value for s in ProfileStrategy])
    ...
    parser.add_argument("--max-eval-segments", type=int, default=None,
                        help="Cap evaluation to first N segments (local smoke test)")
    ...
    for call in calls:
        try:
            ...
            logger.warning(f"{call.call_id}: no profile found for strategy={args.strategy}, skipping")
            continue
