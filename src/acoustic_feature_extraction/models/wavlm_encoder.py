...
import torch
from transformers import Wav2Vec2FeatureExtractor, WavLMForXVector, WavLMModel
...
class WavLMBaseEncoder:
    def __init__(
        ...
        self.model = WavLMModel.from_pretrained(model_name, cache_dir=str(cache_path))
        torch.nn.Module.to(self.model, self.device)
        self.model.eval()
        for param in self.model.parameters():
            param.requires_grad = False
